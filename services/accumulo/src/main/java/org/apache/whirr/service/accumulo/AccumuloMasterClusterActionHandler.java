...
public class AccumuloMasterClusterActionHandler extends AccumuloClusterActionHandler{
  ...
  private void createClientSideHadoopSiteFile(ClusterSpec clusterSpec, Properties config) {
    ...
    File hbaseSiteFile = new File(configDir, AccumuloConstants.FILE_ACCUMULO_SITE_XML);
    try {
      Files.write(generateAccumuloConfigurationFile(config), hbaseSiteFile, Charsets.UTF_8);
      LOG.info("Wrote Accumulo site file {}", hbaseSiteFile);
    } catch (IOException e) {
      LOG.error("Problem writing Accumulo site file {}", hbaseSiteFile, e);
    }
  }
